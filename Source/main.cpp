...
void Level::remove_all_arrows() {
    ...
    auto new_last_entity = std::remove_if(all_arrows.begin(), all_arrows.end(), [](const Arrow* a) -> bool { return a->appear; });
    all_arrows.erase(new_last_entity, all_arrows.end());

   
    l_arrows.remove_if([](const LEFT_ARROW& la) -> bool { return la.appear; });
    r_arrows.remove_if([](const RIGHT_ARROW& ra) -> bool { return ra.appear; });
    u_arrows.remove_if([](const UP_ARROW& ua) -> bool { return ua.appear; });
    d_arrows.remove_if([](const DOWN_ARROW& da) -> bool { return da.appear; });
}
